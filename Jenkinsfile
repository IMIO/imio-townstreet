pipeline {
    ...
    stages {
        stage('Build') {
            ...
            steps {
                sh "fpm --verbose -a amd64 -n imio-townstreet -s python -t deb -v `echo ${VERSION}` --prefix /usr -d passerelle setup.py"
        ...
        stage('Push deb to buster apt repo') {
            ...
            steps {
                withCredentials([usernameColonPassword(credentialsId: 'nexus-teleservices', variable: 'CREDENTIALS'),string(credentialsId: 'nexus-url-buster', variable:'NEXUS_URL_BUSTER')]) {
                    sh ('curl -vk --fail -u $CREDENTIALS -X POST -H Content-Type:multipart/form-data --data-binary @imio-townstreet_`echo ${VERSION}`_amd64.deb $NEXUS_URL_BUSTER')
                }
            }
        }
